...
def buildLanguageCounts(languages, dictSize, minWSize):
...
outFilename = 'report_char_seq2.csv'

#variables to be used for exploration and reporting
randWordSeqLength = (5, 15, 30)
numRandomSentenceTrial = 3
numRandomSeqLengthTrial = 3
sizeOfDict = 40000
...
langCounts = buildLanguageCounts(languages,sizeOfDict, minWordSize)
#for i in range(len(languages)):



#start testing samples for each language
...
#for i in range(0,2):
    ...
    if (numRandomSeqLengthTrial > 0):
        ...
        " random sequence lengths "+str(randWordSeqLength)+"\n")
    print("random word sequences of lengths: ",randWordSeqLength)
    prediction = -1
    randSeqResults=[]
    ...
    for trialNum in range(numRandomSeqLengthTrial):
    ...
    for trialNum in range (numRandomSentenceTrial):
        ...
        for index,lang in enumerate(languages):
            ...
            freqs, counts[index] =convertToFreqCounts(sentence2, langCounts,index)
        print(counts)
